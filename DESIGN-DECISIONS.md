...
### ADR-F3 (2026-06-04) — Checkout self-service `/checkout` (Fase 1) + flag de activación
Nueva ruta `/checkout` + `/checkout/confirmacion` (self-service, **post-reunión y secundaria** al contact-first): captura el lead en MailerLite con los custom fields del journey y muestra instrucciones de transferencia bancaria (confirmación manual, 0% comisión). Autor: **Jose Guerrero (PR #5)**.

- **Arquitectura (puerto/adapter).** El checkout depende del puerto `EmailService` (`lib/email/types.ts`); MailerLite es un adapter por `fetch` REST sin SDK (`lib/email/mailerlite.ts`), seleccionado por factory según `EMAIL_PROVIDER` (`lib/email/index.ts`). Lógica de negocio en funciones puras testeables (`lib/checkout.ts`: `validateCheckout` + `procesarCheckout`); la Server Action (`app/checkout/actions.ts`) es cáscara delgada. Precio **derivado de `STEPS` + `FOUNDER_SPOTS_LEFT`** (un-dato-un-dueño) vía helpers añadidos a `lib/oferta.ts`. Datos bancarios server-side desde env (`lib/bank.ts`, nunca `NEXT_PUBLIC`). Fase 2: swap a Resend para transaccional cambiando 1 adapter + env.
- **Decisión de integración — flag `NEXT_PUBLIC_CHECKOUT_ENABLED` (`lib/flags.ts`).** El flujo depende de secretos de runtime (MailerLite API key, datos bancarios) que **no** estaban configurados en Vercel al integrar. Como `main` auto-despliega a prod, se introdujo el flag (`isCheckoutEnabled()`, **off por defecto**) para que el feature quede **"dark"** sin exponer un flujo roto: botón "Contratar" oculto en `OfferLadderV3` + las rutas `/checkout` redirigen a `/consultoria-legal`. **Activación:** configurar las env de `.env.example` (`MAILERLITE_*`, `BANK_*`, `NOTIFICATION_EMAIL`) en Vercel prod y poner `NEXT_PUBLIC_CHECKOUT_ENABLED=1` + redeploy. El flag fue la adaptación de integración (Alexis); el resto es el PR #5.
- **Followup de privacidad (Fase 1, aceptado).** Con el flag activo, `/checkout/confirmacion` muestra los datos bancarios a cualquier visitante directo (sin gate de checkout completado). Pendiente: gate por token/cookie o mover los datos solo al correo.
- **Specs (fuente única):** `docs/superpowers/specs/2026-06-03-checkout-consultoria-design.md` · `docs/superpowers/specs/2026-06-03-mailerlite-growth-architecture.md`.
